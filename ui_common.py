...
def setup_styles(root):
    ...
    style.configure("StepActive.TLabel", background=BG, foreground=GREEN_DARK, font=("Segoe UI Semibold", 9))
    ...
    style.configure("Horizontal.TProgressbar", troughcolor="#E8EDF4", background=GREEN, thickness=12)
...
def stepper(parent, active, labels):
    ...
    for i, label in enumerate(labels, 1):
        ...
        bubble = tk.Label(
            cell, text=("✓" if i < active else str(i)),
            width=2, height=1,
            bg=(GREEN if i <= active else "#EEF2F7"),
