...
static fs::path auto_find_game_root(const fs::path& exe_dir) {
    ...
    if (looks_like_game_root(exe_dir.parent_path())) return exe_dir.parent_path();
...
static bool load_package(const fs::path& exe_path, std::string& lang_dir, std::vector<PackageFile>& files, std::wstring& error) {
    ...
    char end_magic[8]{};
    if (!read_num(in, archive_len) || !read_exact(in, end_magic, 8) || memcmp(end_magic, END_MAGIC, 8) != 0) {
    ...
    in.seekg(start);
    char pkg_magic[8]{};
    if (!read_exact(in, pkg_magic, 8) || memcmp(pkg_magic, PKG_MAGIC, 8) != 0) {
...
static bool apply_package(const fs::path& root, const std::string& lang_dir_utf8, const std::vector<PackageFile>& files, fs::path& backup_out, std::wstring& error) {
    ...
    for (const auto& pf : files) {
        ...
        if (ec) { error = L"패치 하위 폴더 생성에 실패했습니다."; return false; }
...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR, int) {
    ...
    if (!n || n >= std::size(module_path)) {
        MessageBoxW(nullptr, L"실행 파일 경로를 확인할 수 없습니다.", L"RenPy Korean Patch", MB_ICONERROR);
        CoUninitialize();
        return 1;
    }
    ...
    if (!load_package(exe_path, lang_dir, files, error)) {
        MessageBoxW(nullptr, error.c_str(), L"RenPy Korean Patch", MB_ICONERROR);
        CoUninitialize();
        return 2;
    }
    ...
    if (root.empty()) {
        MessageBoxW(nullptr, L"게임 폴더를 찾지 못해 패치를 취소했습니다.", L"RenPy Korean Patch", MB_ICONWARNING);
        CoUninitialize();
        return 3;
    }
    ...
    MessageBoxW(nullptr, message.c_str(), L"RenPy Korean Patch", MB_ICONINFORMATION);
    CoUninitialize();
    return 0;
}
