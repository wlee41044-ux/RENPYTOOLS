    ...
    for line in lines[1:]:
        ...
        if _is_say_like(line):
            if pending_source is None:
                # A hand-written translation without source comments cannot be
                # reconstructed safely. Omit this unit instead of copying the
                # other language's dialogue into the Korean patch.
                unsafe = True
                break
            translated = translations.get(pending_source, pending_source)
...
def augment_patch_from_existing_tl(patch_root, game_dir, target_dir="korean"):
    ...
    Existing translations are ideal structural templates because their dialogue
    translation identifiers were generated by the exact game/version being patched.
    We keep those identifiers and replace the translated dialogue with RenPy Tools'
    target-language text. This avoids trying to guess Ren'Py translation IDs.
    """
    ...
    for source_file in scripts:
        ...
        for match, block in _translation_blocks(text):
            ...
            if kind in {"python", "style"} or rest:
                # Do not import another language's font/layout customizations.
                # v0.5.2 supplies the Korean font compatibility layer instead.
                continue
        ...
        destination.write_text(
            "# Generated by RenPy Tools v0.5.3\n"
    ...
    if generated_files == 0 or dialogue_blocks == 0:
        # A strings-only/hand-written language folder gives us no reliable
        # dialogue identifiers, so keep the old fallback untouched.
        return None

    # Keep only source strings that were not already represented by the template
    # and were not handled by a dialogue translation unit. This avoids duplicate
    # `old` registrations while retaining custom UI strings found by our scanner.
    extras = [
