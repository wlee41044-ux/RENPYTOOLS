...
static bool looks_like_game_root(const fs::path& root) {
    ...
    for (const auto& entry : fs::directory_iterator(game, fs::directory_options::skip_permission_denied, ec)) {
        ...
        if (ext == L".rpa" || ext == L".rpy" || ext == L".rpyc" || ext == L".rpym" || ext == L".rpymc") return true;
    }
    return true;
}
...
static fs::path normalize_selected_root(fs::path p) {
    ...
    if (p.filename() == L"game" && fs::is_directory(p, ec) && looks_like_game_root(p.parent_path())) return p.parent_path();
    return {};
}
...
struct PackageFile { std::string relative_path; std::vector<char> data; };
...
template <typename T>
static bool read_num(std::ifstream& in, T& value) { return read_exact(in, &value, sizeof(T)); }
...
static bool load_package(const fs::path& exe_path, std::string& lang_dir, std::vector<PackageFile>& files, std::wstring& error) {
    ...
    if (!in) { error = L"패치 실행 파일을 읽을 수 없습니다."; return false; }
    in.seekg(0, std::ios::end);
    std::streamoff file_size = in.tellg();
    if (file_size < 16) { error = L"패치 데이터가 없습니다."; return false; }
    ...
    if (!read_num(in, archive_len) || !read_exact(in, end_magic, 8) || std::memcmp(end_magic, END_MAGIC, 8) != 0) {
        error = L"유효한 RenPy Tools 독립 패치 파일이 아닙니다."; return false;
    }
    if (archive_len > static_cast<uint64_t>(file_size - 16)) { error = L"패치 데이터 길이가 잘못되었습니다."; return false; }
    std::streamoff start = file_size - 16 - static_cast<std::streamoff>(archive_len);
    in.seekg(start);
    char pkg_magic[8]{};
    if (!read_exact(in, pkg_magic, 8) || std::memcmp(pkg_magic, PKG_MAGIC, 8) != 0) { error = L"패치 데이터 헤더가 손상되었습니다."; return false; }
    uint32_t count = 0;
    uint16_t lang_len = 0;
    if (!read_num(in, count) || !read_num(in, lang_len) || lang_len == 0 || lang_len > 128 || count > 200000) {
        error = L"패치 데이터 형식이 올바르지 않습니다."; return false;
    }
    lang_dir.resize(lang_len);
    if (!read_exact(in, lang_dir.data(), lang_len)) { error = L"언어 정보를 읽지 못했습니다."; return false; }
    files.clear(); files.reserve(count);
    for (uint32_t i = 0; i < count; ++i) {
        uint32_t path_len = 0; uint64_t data_len = 0;
        if (!read_num(in, path_len) || !read_num(in, data_len) || path_len == 0 || path_len > 4096 || data_len > 1024ULL * 1024ULL * 256ULL) {
            error = L"패치 파일 정보가 손상되었습니다."; return false;
        }
        PackageFile pf;
        pf.relative_path.resize(path_len);
        if (!read_exact(in, pf.relative_path.data(), path_len)) { error = L"패치 경로를 읽지 못했습니다."; return false; }
        pf.data.resize(static_cast<size_t>(data_len));
        if (data_len && !read_exact(in, pf.data.data(), static_cast<size_t>(data_len))) { error = L"패치 파일을 읽지 못했습니다."; return false; }
...
static bool safe_relative(const fs::path& p) {
    ...
    for (const auto& part : p) if (part == L".." || part == L".") return false;
    return true;
}

static bool migrate_legacy_backup(const fs::path& root, const fs::path& game, std::wstring& error) {
    std::error_code ec;
    fs::path legacy = game / L"_RenPyTools_Backup";
    if (!fs::exists(legacy, ec)) return true;
    fs::path backup_base = root / L"_RenPyTools_Backup";
    ec.clear();
    fs::create_directories(backup_base, ec);
    if (ec) { error = L"기존 백업을 game 폴더 밖으로 옮길 준비에 실패했습니다.\n" + widen_utf8(ec.message()); return false; }
    fs::path dest = backup_base / (L"legacy_inside_game_" + timestamp_now());
    ec.clear();
    fs::rename(legacy, dest, ec);
    if (!ec) return true;

    ec.clear();
    fs::copy(legacy, dest, fs::copy_options::recursive | fs::copy_options::overwrite_existing, ec);
    if (ec) { error = L"기존 game 내부 백업을 안전한 위치로 복사하지 못했습니다.\n" + widen_utf8(ec.message()); return false; }
    ec.clear();
    fs::remove_all(legacy, ec);
    if (ec) { error = L"기존 game 내부 백업을 제거하지 못했습니다. 게임 실행 전에 game/_RenPyTools_Backup 폴더를 수동으로 밖으로 옮겨주세요.\n" + widen_utf8(ec.message()); return false; }
    return true;
}

static bool apply_package(const fs::path& root, const std::string& lang_dir_utf8, const std::vector<PackageFile>& files, fs::path& backup_out, std::wstring& error) {
    std::error_code ec;
    fs::path game = root / L"game";
    if (!fs::is_directory(game, ec)) { error = L"선택한 위치에 game 폴더가 없습니다."; return false; }
    if (!migrate_legacy_backup(root, game, error)) return false;

    fs::path lang_dir = fs::path(widen_utf8(lang_dir_utf8));
    if (lang_dir.empty() || lang_dir.has_parent_path()) { error = L"패치 언어 폴더 정보가 올바르지 않습니다."; return false; }
    ...
    if (fs::exists(target, ec)) {
        ec.clear();
        fs::create_directories(backup_base, ec);
        if (ec) { error = L"백업 폴더를 만들 수 없습니다.\n" + widen_utf8(ec.message()); return false; }
        backup_out = backup_base / (lang_dir.wstring() + L"_" + timestamp_now());
        ec.clear();
        fs::rename(target, backup_out, ec);
        if (ec) { error = L"기존 번역 폴더 백업에 실패했습니다. 게임이 실행 중인지 확인하세요.\n" + widen_utf8(ec.message()); return false; }
    }

    ec.clear();
    fs::create_directories(target, ec);
    if (ec) { error = L"번역 폴더를 만들 수 없습니다.\n" + widen_utf8(ec.message()); return false; }

    for (const auto& pf : files) {
        fs::path rel = fs::path(widen_utf8(pf.relative_path));
        if (!safe_relative(rel)) { error = L"안전하지 않은 패치 경로가 포함되어 있습니다."; return false; }
        fs::path dest = target / rel;
        ec.clear();
        fs::create_directories(dest.parent_path(), ec);
        if (ec) { error = L"패치 하위 폴더 생성에 실패했습니다.\n" + widen_utf8(ec.message()); return false; }
        std::ofstream out(dest, std::ios::binary | std::ios::trunc);
        if (!out) { error = L"패치 파일을 쓸 수 없습니다: " + dest.wstring(); return false; }
        if (!pf.data.empty()) out.write(pf.data.data(), static_cast<std::streamsize>(pf.data.size()));
        if (!out.good()) { error = L"패치 파일 쓰기에 실패했습니다: " + dest.wstring(); return false; }
    }
    ...
    if (!loader_out) { error = L"번역 언어 활성화 파일을 쓸 수 없습니다."; return false; }
    loader_out << "# Generated by RenPy Tools v0.5.13\n";
    loader_out << "# Activates the installed language before normal game init.\n";
    loader_out << "define config.language = '" << lang_dir_utf8 << "'\n";
    if (lang_dir_utf8 == "korean") {
        loader_out << "\ntranslate korean python:\n";
        loader_out << "    try:\n";
        loader_out << "        gui.language = 'korean-with-spaces'\n";
        loader_out << "    except Exception:\n";
        loader_out << "        pass\n";
    }
    if (!loader_out.good()) { error = L"번역 언어 활성화 파일 쓰기에 실패했습니다."; return false; }
    return true;
}
