...
class Translator:
    ...
    def translate(self, text):
        ...
        if self.provider == "무료 Google 번역":
            ...
            out = self.openai(
                masked,
                self.url or "http://127.0.0.1:11434/v1/chat/completions",
                self.model or "qwen2.5:7b",
                self.key or "ollama",
            )
        elif self.provider == "LM Studio / OpenAI 호환":
            out = self.openai(
                masked,
                self.url or "http://127.0.0.1:1234/v1/chat/completions",
                self.model or "local-model",
                self.key or "lm-studio",
            )
        else:
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": (
                            "Translate visual novel text. Return only translated text. "
                            "Preserve every __RPTOKEN_n__ placeholder exactly."
                        ),
                    },
...
class PatcherApp(tk.Tk):
    ...
    def page_folder(self):
        outer, card_body = card(self.container)
        outer.pack(fill="x")
        ttk.Label(card_body, text="📁  번역할 게임 폴더 선택", style="Section.TLabel").pack(anchor="w")
        ttk.Label(
            card_body,
            text="Extractor에서 만든 Decompiled 폴더도 바로 선택할 수 있습니다.",
            style="Muted.Card.TLabel",
        ).pack(anchor="w", pady=(3, 12))

        row = ttk.Frame(card_body, style="Card.TFrame")
        row.pack(fill="x")
        ...
        ttk.Label(card_body, textvariable=self.scan, style="Muted.Card.TLabel").pack(anchor="w", pady=(10, 0))
    ...
    def page_work(self):
        ...
        ttk.Label(card_body, text="번역 및 패치 생성 중", style="Section.TLabel").pack(anchor="w")
        ttk.Label(card_body, textvariable=self.status, style="Muted.Card.TLabel").pack(anchor="w", pady=(3, 12))
        self.bar = ttk.Progressbar(card_body, mode="determinate")
        self.bar.pack(fill="x")
        self.percent = ttk.Label(card_body, text="0%", style="Muted.Card.TLabel")
        self.percent.pack(anchor="e", pady=(4, 8))
        self.details = ttk.Button(card_body, text="자세히 보기", style="Flat.TButton", command=self.toggle_log)
        self.details.pack(anchor="w")
        self.log = tk.Text(card_body, height=12, wrap="word", state="disabled", relief="flat", bg="#F8FAFD")
    ...
    def worker(self):
        ...
        try:
            ...
            translator = Translator(
                options["provider"],
                SOURCE_CODES[options["source_lang"]],
                target_code,
                options["api_key"],
                options["base_url"],
                options["model"],
            ...
            for index, (file_path, no, source_text) in enumerate(items, 1):
                if source_text in memory:
                    translated = memory[source_text]
                else:
                    last_error = None
                    for attempt in range(3):
                        ...
                        self.add_log(f"[번역 실패] {file_path.name}:{no} · {last_error}")
                    memory[source_text] = translated

                grouped.setdefault(file_path, []).append((no, source_text, translated))
                self.progress(index, total, f"번역 중 · {index}/{total}")
            ...
            for file_path, entries in grouped.items():
                ...
                for no, old, new in entries:
                    lines.extend(
                        [
                            f"    # {rel.as_posix()}:{no}",
                            f'    old "{escape_rpy(old)}"',
                            f'    new "{escape_rpy(new)}"',
                            "",
                        ]
                    )
            ...
            with zipfile.ZipFile(self.output_zip, "w", zipfile.ZIP_DEFLATED) as archive:
    ...
    def page_done(self):
        outer, card_body = card(self.container)
        outer.pack(fill="x")
        ttk.Label(card_body, text="✓  한글패치 생성 완료", style="Section.TLabel").pack(anchor="center")
        ttk.Label(
            card_body,
            text=f"처리한 번역 후보 {getattr(self, 'result', 0)}개",
            style="Muted.Card.TLabel",
        ).pack(anchor="center", pady=(5, 12))
        ttk.Label(card_body, text=str(self.output_zip), style="Muted.Card.TLabel").pack(anchor="center")
